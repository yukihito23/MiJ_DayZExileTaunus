...
if (_timeTilRestart < _lockTime) then
{
	if !(ExileServerIsLocked) then
	{
		...
		["toastRequest",["ErrorTitleAndText", ["Server will restart now!", format["You will be kicked off the server due to a restart."]]]] call ExileServer_system_network_send_broadcast;
